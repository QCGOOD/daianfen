    ...
    <scroller v-if="scheduleList.length > 0" :on-infinite="loadMore" :on-refresh="refresh" ref="myscroller">
      <div>
        <div 
          ...
          :class="{ expire: date(item.endTime, item.startTime) == 1, today: date(item.endTime, item.startTime) == 2 }">
          <p class="date">{{item.startTime | dateFormat}}</p>
          <!--  至 {{item.endTime | dateFormat}} -->
...
export default {
  ...
  methods: {
    ...
    toDetail(id, type) {
      this.$http.get('/courseClassLesson/get', {id: id})
      .then(res => {
        if(type == 3){
          this.$router.push(`/curriculum/detail?id=${res.data.data.courseId}`)
        }else if(type == 1) {
          this.$router.push(`/activity/detail?id=${res.data.data.courseId}`)
        }
      })
    },
    date(value, start) {
      ...
      // 结束时间  
      let endTime = value.substr(11, 5);
      let dateTime = Date.parse(new Date(date+' '+endTime));
      // 当前时间戳
  ...
  //       this.searchData.pageNum = page + 1;
    ...
    getScheduleList() {
      ...
      this.$http.get('/memberCalendarEvent/page', this.searchData)
      .then(res => {
