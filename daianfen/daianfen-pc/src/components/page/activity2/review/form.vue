<template>
  <div class="wrap" ref="wrap" v-loading="dataLoading">
    <div ref="header">
      <qc-header :title="$route.query.id?'修改活动回顾':'添加活动回顾'"></qc-header>
    </div>
    <div class="area-box" :style="{'height': `${areaHeight}px`}">
      <!-- 选择课程 -->
      <el-form class="form" :model="model" ref="model" label-position="top" size="small">
        <el-form-item label="选择活动" prop="sourceId" :rules="[{ required: true, message: '该字段不能为空'}]">
          <el-select size="small" :disabled="$route.query.id?true:false" v-model="actLesson" @change="curLesson">
            <el-option v-for="(item, index) in lessonList" :key="index" :label="item.title" :value="index" ></el-option>
          </el-select>
        </el-form-item>
        <!-- 金句 开始-->
    ...
    <image-dialog :visible="digImgWrap" uploadType="radio" imageType='actReview' @close-imgbox="closeImgbox" @get-imgsrc="onGetImgsrc"></image-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      model: {
        ...
        shortSentenceList: [],
        content: '',
        qrcodeUrlString:'http://abs.wego168.com/accelerator_test/mobile/POUND/curriculum/slectGolden?wo=1&wot=2&woacm=1&mpl=1&sourceType=1&id=',
        // qrcodeUrlString:'http://192.168.1.92:8080/#/curriculum/slectGolden?id='
      },
      imgList: [],
      uploadType: 'radio',
      limit: 1,
  ...
  methods: {
    ...
    curLesson(i) {
      this.model.sourceId = this.lessonList[i].id
      this.model.title = this.lessonList[i].title
    },
    listActivity() {
    ...
    getDetail(id) {
      api.lessonReviewDetail({id: id}).then(res => {
        console.log('详情',res.data.data)
        this.model = res.data.data
        // 显示已选活动名
        this.actLesson = res.data.data.title
    ...
    updete() {
      api.updeteLessonReview(this.model).then(res => {
        this.loading = false;
        this.$message({
          message: '保存成功',
          type: 'success'
    ...
    addReview() {
      api.saveLessonReview(this.model).then(res => {
    ...
    submit () {
      ...
      this.$refs['model'].validate((valid) => {
        if (valid) {
          this.loading = true;
          if(this.$route.query.id) this.updete()
          else this.addReview()
        } else {
          return false;
        }
