<template>
  <div class="wrap" ref="wrap" v-loading="dataLoading">
    <div ref="header">
      <qc-header :title="$route.query.id?'修改课程回顾':'添加课程回顾'"></qc-header>
    </div>
    <div class="area-box" :style="{'height': `${areaHeight}px`}">
      <!-- 选择课程 -->
      <el-form class="form" :model="model" ref="model" label-position="top" size="small">
        <el-form-item label="选择课程" prop="sourceId" :rules="[{ required: true, message: '该字段不能为空'}]">
  ...
  data () {
    return {
      ...
      model: {
        ...
        shortSentenceList: [],
        content: '',
        qrcodeUrlString:'http://abs.wego168.com/accelerator_test/mobile/POUND/curriculum/slectGolden?wo=1&wot=2&woacm=1&mpl=1&sourceType=3&id=',
